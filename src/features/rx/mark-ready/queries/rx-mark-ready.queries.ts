...
import { markRxReady } from "../api/rx-mark-ready.api";
import { MARK_READY_ERROR_MESSAGES } from "../lib/rx-mark-ready.errors";
...
export function useMarkRxReadyMutation() {
    const qc = useQueryClient();

    return useToastMutation({
        mutationFn: markRxReady,
        toastMessages: {
            loading: "Rezept wird als fertig vorbereitet markiert...",
            success: "Rezept wurde als fertig vorbereitet markiert",
            error: (error) =>
                getApiErrorMessage(error, {
                    fallback:
                        "Rezept konnte nicht als fertig vorbereitet markiert werden",
                    codeMap: MARK_READY_ERROR_MESSAGES,
                    statusMap: {
